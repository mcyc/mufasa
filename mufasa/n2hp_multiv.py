# Modified version of ammonia_multiv.py
...
def n2hp_multi_v_model_generator(n_comp, linenames = None):
    ...
    ----------
    n_comp : int
        The number of velocity components to fit
    linenames : list
        A list of line names from the set ('onezero','twoone','threetwo'); default is just 'onezero'
    Returns
    -------
    ...
    def n2hp_vtau_multimodel(xarr, *args):
        # the parameters are in the order of vel, width, tex, tau for each velocity component
        assert len(args) == n_para
        return n2hp_multi_v(xarr, *args, line_names=linenames)

    mod = model.SpectralModel(n2hp_vtau_multimodel, n_para,
            parnames=[x
                      for ln in idx_comp
...
def n2hp_multi_v(xarr, *args, **kwargs):
    ...
    for vel, width, tex, tau in zip(args[::4], args[1::4], args[2::4], args[3::4]):
        ...
        model_spectrum = _n2hp_spectrum(xarr, tex=tex, tau_dict=tau_dict, width=width, xoff_v=vel,
...
def _n2hp_spectrum(xarr, tex, tau_dict, width, xoff_v, line_names, background_ta=0.0, fillingfraction=None,
    ...
    """
    Helper function: given a dictionary of N2H+ optical depths, an excitation temperature... etc, and produce a
    ...
    for linename in line_names:
        ...
        for kk,nuo in enumerate(nuoff):
            tauprof_ = (tau_dict[linename] * tau_wts[kk] * np.exp(-(xarr.value+nuo-lines[kk])**2 / (2.0*nuwidth[kk]**2)))
            if return_components:
